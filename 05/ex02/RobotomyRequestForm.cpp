...
/*   Updated: 2022/08/11 09:24:22 by jbrown           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		RobotomyRequestForm::execute( Bureaucrat const &executor ) const{
	...
	if (executor.GetGrade() <= this->getExeGrade())
	{
		...
		if (random)
		{
			std::cout << executor.GetName() << " has joined the ranks of our"
			<< " wonderful robot overlords." << std::endl;
		}
		else
		{
			std::cout << "Drill was jammed. " << executor.GetName()
			<< " will have to remain human a little while longer." << std::endl;
		}
	}
	else
	{
		std::cout << executor.GetName() << " doesn't have what it takes to be a"
